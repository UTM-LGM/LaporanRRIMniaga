...
@Component({
  selector: 'app-receipt-skrap',
  templateUrl: './receipt-skrap.component.html',
  styleUrls: ['./receipt-skrap.component.css']
})
export class ReceiptSkrapComponent {
    ...
    this.activateRoute.queryParams.subscribe(params => {
      if (params['listTapper']) {
        ...
        for (const transaction of this.pekebun) {
          if (transaction.checked && transaction.type == 'Skrap') {
            this.selectedSkrap.push(transaction);
      ...
      if (params['pekebun']) {
        ...
        for (const pekebun of this.pekebun) {
          if (pekebun.checked && pekebun.type == 'Skrap') {
  ...
  //   const sum = year + parseInt(month, 10) + parseInt(day, 10);
  ...
  //     this.padTo2Digits(this.date.getHours()) +
  //     this.padTo2Digits(this.date.getMinutes()) +
  //     this.padTo2Digits(this.date.getSeconds())
  //   // this.globalService.getUserPremiseId();
  ...
  generateReceiptNo() {
    ...
    this.receiptNumber = "W" + formattedYear + month + day + lastDigit + hours + minutes + seconds + "1";
    console.log(this.receiptNumber);
